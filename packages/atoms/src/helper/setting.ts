...
export const createSettingAtom = <T extends object>(
  ...
  const [, , useSettingValue, , getSettings, setSettings] = createAtomHooks<T>(atom as any)
  ...
  >(
    ...
    // @ts-expect-error
    return useAtomValue(useMemo(() => selectAtom(atom, stableSelector.current), [stableSelector]))
  }
